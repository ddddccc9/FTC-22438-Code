...
@TeleOp(name = "Capstone_Stay")

public class CapstoneProject_stay extends LinearOpMode{
    private DcMotor lf;
    private DcMotor lb;
    private DcMotor rf;
    private DcMotor rb;
...
//        pid3.limit(0.5);
    ...
    public double move(double x,double y,double angle,double power){
        ...
        double lf_0 = power * (y + (x + angle));
        double lb_0 = power * (y - (x - angle));
        double rf_0 = power * (y - (x + angle));
        double rb_0 = power * (y + (x - angle));

        if(lf_0<0.12 && lb_0<0.12 && rf_0<0.12 && rb_0 <0.12 ){
            lf.setPower(0);
            lb.setPower(0);
    ...
    public void moveTo(int target_x,int target_y,int target_angle,boolean Flag){
        ...
        while(true){
            ...
            double output = move(move_x,move_y,move_angle,1);
